...
<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    close: void;
    resolve: { confirm: boolean; value?: ActivityDirectiveDeletionMap };
  }>();
  ...
  function confirmDelete() {
    ...
    const deletionMap = Object.keys(activityDirectiveDeletionMap).reduce(
    ...
    open = false;
    dispatch('resolve', { confirm: true, value: deletionMap });
  }
...
<StellarDialog
  ...
  size="auto"
  className="w-[480px]"
  title="Delete Activity {activityIds.length > 1 ? 'Directives' : 'Directive'}"
  on:close
>
