...
<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    close: void;
    resolve: { confirm: boolean; value?: ModelSlim };
  }>();
  ...
  $: confirmDisabled = !selectedMissionModel || !$planMergeRequestsIncoming || $planMergeRequestsIncoming.length > 0;
  ...
  function handleConfirm() {
    if (selectedMissionModel !== null) {
      open = false;
      dispatch('resolve', { confirm: true, value: selectedMissionModel });
    }
  }
</script>

<StellarDialog bind:open size="lg" className="h-[80vh] w-[80vw] max-w-[none]" title="Change Mission Model" on:close>
  <div class="flex max-h-[70vh] flex-1 flex-col overflow-hidden p-0.5">
  ...
  <svelte:fragment slot="footer">
    <div class="flex w-full items-center justify-between">
      <div class="text-sm text-muted-foreground">Snapshot will be automatically created</div>
      <StellarDialogActionButtons
        confirmText="Change Mission Model"
        {confirmDisabled}
