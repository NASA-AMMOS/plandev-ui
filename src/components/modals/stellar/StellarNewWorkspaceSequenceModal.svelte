...
<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    close: void;
    resolve: { confirm: boolean; value?: { filePath: string } };
  }>();

  let filePath: string = joinPath([currentWorkspace?.name ?? '', startingPath]);
  let fileName: string = '';

  $: confirmButtonDisabled = fileName.trim() === '';
  ...
  function handleConfirm() {
    if (!confirmButtonDisabled) {
      ...
      dispatch('resolve', {
        ...
        value: {
          filePath: cleanPath(
            joinPath([filePath.replace(new RegExp(`^${currentWorkspace?.name}`), '.'), fileName.trim()]),
          ),
        },
      });
    }
...
<StellarDialog bind:open size="md" title="New File" on:close>
