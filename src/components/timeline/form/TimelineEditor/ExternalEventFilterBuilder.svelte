...
<script lang="ts">
  ...
  // stays mostly the same for events
  ...
  // mostly the same as the activity one
  function onAddDynamicFilter(list: 'dynamic_type_filters' | 'global_filters') {
    const field = list === 'dynamic_type_filters' ? 'Type' : 'Name'; // only consider these 2 because it's the default filter. this doesn't mean the other filters aren't dynamic filters (as the dynamic filter change function does fire for the other filters when updated)
                      ...
                      {#if filteredExternalEventTypes.length > 0}
                        <MenuItem on:click={() => onAddAllManualTypes()}>
                          <div class="st-typography-bold manual-types-add-all">
                            Add {filteredExternalEventTypes.length !== $activityTypes.length ? 'Matching' : 'All'} +
                          </div>
                        </MenuItem>
