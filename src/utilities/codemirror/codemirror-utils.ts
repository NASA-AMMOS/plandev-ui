...
export function isVariableDeclaration(arg: object): arg is VariableDeclaration {
  return 'type' in arg;
}
...
export function getMissingVariableDefs(varInfoArray: VariableTextDef[]): VariableDeclaration[] {
  return varInfoArray
    .filter((varInfo): varInfo is { varDef: VariableDeclaration } => !varInfo.node && !!varInfo.varDef)
    .map(varInfo => varInfo.varDef);
}
