...
/**
 * Gets the token for WebSocket authentication.
 * Supports both OIDC (accessToken cookie) and legacy (base64 user cookie) formats.
 */
function getToken(): string {
  if (browser && document?.cookie) {
    ...
    const accessToken = cookies.find(entry => entry.startsWith('accessToken='));
    if (accessToken) {
      return accessToken.split('accessToken=')[1];
    }

    // Legacy format: base64-encoded user cookie containing token
